...
//initiate the smoothScroll functionality for anchor links
smoothScroll.init({
...
$(document).ready(function (){
  var mqsd = window.matchMedia('screen and (max-width: 991px)');

  // if the screen width is less than 767px and
  // the sidebar has a child div named with classes
  // of "nav" and "sidenav-menu" convert the sidenav
  // menu to a dropdown menu
  if (mqsd.matches && $('.sidebar > .nav.sidenav-menu').length) {
    // capture the text contained in the h2 tag as html
    ...
    // convert the h2 tag in the sidebar to a button
  ...
  // set headline margins within parallax jumbotrons
  var jumboHeight = $('.jumbotron').height();

  $('.row.headline-row').css('margin-top', jumboHeight / 1.65);

  $('iframe,object,embed').each(function() {
    // remove the height and width of the video
    // and give it a class name
    $(this).addClass('embed-responsive-item');
    $(this).removeAttr('height');
    $(this).removeAttr('width');

    // measure the aspect ratio of the video object
    // then give it the appropriate wrapper div
    if ($(this).innerHeight() === $(this).width() * 0.75) {
      $(this).wrap('div class="embed-responsive embed-responsive-4by3"></div>');
    } else {
      $(this).wrap('<div class="embed-responsive embed-responsive-16by9"></div>');
    }

    // if it's a youtube video and doesn't have
    // rel set to 0, do that
    var videoURL = $(this).attr('src');
    if (videoURL.indexOf('rel=0') === -1) {
      $(this).attr('src', videoURL + '?rel=0');
    }
  });

  if ($('.embed-responsive-16by9').length || $('.embed-responsive-4by3').length) {
    if ($(this).parent('p')) {
      $(this).unwrap();
    }
  }
